...
class FtpConnection extends EventEmitter {
	...
	login(username, password) {
		return Promise.try(() => {
			...
			if (!loginListeners || !loginListeners.length) {
				...
				return this.server.emitPromise('login', {connection: this, username: username, password: password});
			}
		})
			.then(({root, cwd, fs, blacklist = [], whitelist = []} = {}) => {
				...
				this.fs = fs || new FileSystem(this, {root: root, cwd: cwd});
			});
	}
	...
	reply(options = {}, ...letters) {
		const satisfyParameters = () => {
			...
			} // allow passing in code as first param
			...
			return Promise.map(letters, (promise, index) => {
				return Promise.resolve(promise)
					.then((letter) => {
						...
						if (!letter.socket) {
							letter.socket = options.socket ? options.socket : this.commandSocket;
						}
						if (!letter.message) {
							letter.message = DEFAULT_MESSAGE[options.code] || 'No information';
						}
						if (!letter.encoding) {
							letter.encoding = this.encoding;
						}
