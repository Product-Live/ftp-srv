...
class FileSystem {
	...
	write(fileName, {append = false, start = undefined} = {}) {
		...
		const stream = fs.createWriteStream(fsPath, {flags: !append ? 'w+' : 'a+', start: start});
	...
	read(fileName, {start = undefined} = {}) {
		...
		return fs.statAsync(fsPath)
			...
			.then(() => {
				const stream = fs.createReadStream(fsPath, {flags: 'r', start: start});
				return {
					stream: stream,
					clientPath: clientPath
				};
	...
	delete(path) {
		...
		return fs.statAsync(fsPath)
			.then((stat) => {
				if (stat.isDirectory()) {
					return fs.rmdirAsync(fsPath);
				}
				return fs.unlinkAsync(fsPath);
			});
	}
